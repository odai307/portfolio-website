      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-12 max-w-4xl mx-auto">
          {experiences.map((exp, idx) => (
            <div
              key={idx}
              className="glass-panel glass-panel-hover rounded-2xl p-6 sm:p-8 border border-white/10 relative"
            >
              {/* Top Meta info */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-6 mb-6">
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
                    {exp.company}
                    <span className="text-xs px-2.5 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 font-mono font-normal">
                      Contract
                    </span>
                  </h3>
                  <div className="text-indigo-400 font-medium text-sm mt-1">{exp.title}</div>
                </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                {exp.highlights.map((h, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="p-4 rounded-xl bg-slate-900/40 border border-slate-800/80 hover:border-indigo-500/30 transition-colors"
                    >
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-800/80">
                <div className="flex flex-wrap gap-2">
                  {exp.techStack.map((tech) => (
                    <span
                      key={tech}
                      className="px-2.5 py-1 rounded-md bg-slate-900 text-slate-300 font-mono text-xs border border-slate-800"
                    >
                      {tech}
                    </span>
                  ))}
                ...
                <div className="flex items-center gap-3">
                  {exp.githubUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 hover:text-white text-xs font-medium flex items-center gap-1.5 transition-colors"
                    >
                  ...
                  {exp.liveUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="px-3 py-1.5 rounded-lg bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 hover:bg-indigo-600 hover:text-white text-xs font-medium flex items-center gap-1.5 transition-colors"
                    >
